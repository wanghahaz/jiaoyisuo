<template>
  <view class="content">
    ...
    <view class="url pd23">
      <view class="ft6 copy_code">
        <view>
          <text>邀请码:</text>
          <text>{{ codeUrl }}</text>
        </view>
        <view @click="copy(invateData.invateAddress)">
          复制邀请码
        </view>
      </view>
     <!-- <view class="code bb">
      ...
      <view class="pd23">
        ...
        <view class="listItem bb" v-for="(val, index) in list" :key="index">
          <view class="head">
            ...
            <view :style="{ color: val.checkStatus < 2 ? '#fff' : '', background: val.checkStatus < 2 ? '#D8D8D8' : '' }">
              {{ val.checkStatus == -1 ? '未通过' : val.checkStatus == 0 ? '未认证' : val.checkStatus == 1 ? '待审核' : '已认证' }}
            </view>
          </view>
          <view>{{ val.id }}</view>
...
export default {
  ...
  async onLoad() {
    ...
    this.codeUrl = invateData.data.invateAddress.split('=')[1];
