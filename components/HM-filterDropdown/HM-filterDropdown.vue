<template>
	<view class="HMfilterDropdown" @touchmove.stop.prevent="discard" >
		<view class="nav">
			<block v-for="(item,index) in menu" :key="index" v-if="type == 1">
				<view class="first-menu" :class="{'on':showPage==index}" @tap="togglePage(index)">
					<text class="type" v-if="index == 0">市场：</text><text class="type" v-else-if="index == 1">委托类型：</text><text class="type" v-else>委托状态：</text><text class="name">{{item.name}}</text>
					<text class="iconfont triangle" :style="'transform:rotate('+triangleDeg[index]+'deg);'"></text>
				</view>
			</block>
			<block v-for="(item,index) in menu" :key="index" v-if="type != 1">
				<view class="first-menu" :class="{'on':showPage==index}" @tap="togglePage(index)" v-if="index != 2">
					<text class="type" v-if="index == 0">市场：</text><text class="type" v-else-if="index == 1">委托类型：</text><text class="type" v-else>委托状态：</text><text class="name">{{item.name}}</text>
					<text class="iconfont triangle" :style="'transform:rotate('+triangleDeg[index]+'deg);'"></text>
				</view>
			</block>
